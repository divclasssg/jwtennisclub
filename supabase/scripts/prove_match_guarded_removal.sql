...
\if :{?task8_validation_ref}
\else
  \echo 'task8_validation_ref is required'
  \quit 3
\endif
...
select pg_catalog.set_config(
  'app.task8_validation_project_ref',
...
declare
  target_ref text :=
    current_setting('app.task8_validation_project_ref', true);
  release_row match.release_state%rowtype;
  table_row record;
  table_count bigint;
begin
  if target_ref is null or btrim(target_ref) = '' then
    raise exception 'validation project ref is required';
  end if;

  if target_ref = 'ydiusirreirhbvlftegp' then
