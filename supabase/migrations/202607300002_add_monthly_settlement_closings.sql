...
begin
  ...
  if exists (
    ...
    where members.activity_start_month is null
      and members.joined_date <= period_month_end
      and (
  ...
  if normalized_period_month = date '2026-07-01' then
    ...
    if opening_ledger_balance is null then
  ...
  select coalesce(pg_catalog.sum(fee_payments.amount), 0)::bigint
  into actual_fee_income
  ...
  select
    coalesce(pg_catalog.sum(expenses.amount), 0)::bigint,
    count(*)
  into expense_total, expense_count
  from public.expenses as expenses
  where expenses.expense_date between normalized_period_month and period_month_end;
  ...
  into expense_category_rows
  from (
    select
      expenses.category,
      count(*) as count,
      pg_catalog.sum(expenses.amount)::bigint as amount
    from public.expenses as expenses
    where expenses.expense_date between normalized_period_month and period_month_end
  ...
  select coalesce(
    pg_catalog.jsonb_agg(
      pg_catalog.jsonb_build_object(
        'expense_date', public_expenses.expense_date,
        'category', public_expenses.category,
        'description', public_expenses.description,
        'amount', public_expenses.amount
      )
      order by
        public_expenses.expense_date,
        public_expenses.category,
        public_expenses.description,
        public_expenses.id
    ),
    '[]'::jsonb
  )
...
declare
  ...
  closing_id uuid;
  closing_occurred_at timestamptz := pg_catalog.clock_timestamp();
begin
  if requested_period_month is null
    or requested_period_month <> normalized_period_month
  then
...
declare
  ...
  reopen_occurred_at timestamptz := pg_catalog.clock_timestamp();
begin
  if requested_period_month is null
    or requested_period_month <> normalized_period_month
  then
