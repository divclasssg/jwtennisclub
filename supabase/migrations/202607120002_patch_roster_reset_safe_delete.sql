...
begin
  ...
  if position('delete from public.fee_payments;' in function_definition) > 0 then
    patched_definition := replace(
      function_definition,
      'delete from public.fee_payments;',
      'delete from public.fee_payments where true;'
    );
  elsif position('delete from public.fee_payments where true;' in function_definition) > 0 then
  ...
  if position('delete from public.members;' in function_definition) > 0 then
    patched_definition := replace(
      function_definition,
      'delete from public.members;',
      'delete from public.members where true;'
    );
  elsif position('delete from public.members where true;' in function_definition) > 0 then
    patched_definition := function_definition;
  else
