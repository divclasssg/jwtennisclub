...
alter table public.members
  add column pause_start_month date;

update public.members
set pause_start_month = date '2026-08-01'
where status = 'paused';

alter table public.members
  add constraint members_pause_start_month_is_month
  check (
    pause_start_month is null
    or pause_start_month = date_trunc('month', pause_start_month)::date
  ),
  add constraint members_pause_start_month_matches_status
  check (
    (status = 'paused' and pause_start_month is not null)
    or (status <> 'paused' and pause_start_month is null)
  );
...
as $$
  with operator_permissions as (
    ...
    select
      exists (
        ...
        where permission = 'members.view'
      ) as can_view,
      exists (
        select 1
        from operator_permissions
        where permission = 'members.create'
      ) as can_create,
      exists (
        select 1
        from operator_permissions
        where permission = 'members.update'
      ) as can_update,
      exists (
        select 1
        from operator_permissions
        where permission = 'members.contacts.manage'
      ) as can_manage_contacts
  )
  select pg_catalog.jsonb_build_object(
    ...
    'members', case when access.can_view then coalesce((
      ...
      from public.members
      left join public.member_groups
        on member_groups.id = members.group_id
      left join public.member_contacts
        on member_contacts.member_id = members.id
      ...
      where (
        ...
        or members.status::text = requested_status
      )
        and (
          nullif(pg_catalog.btrim(search_query), '') is null
          or members.name ilike
            '%' || pg_catalog.btrim(search_query) || '%'
          or members.member_code ilike
            '%' || pg_catalog.btrim(search_query) || '%'
        )
