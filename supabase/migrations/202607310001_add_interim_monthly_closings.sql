...
create index monthly_closings_period_kind_history_idx
on public.monthly_closings(period_month, closing_kind, version desc);
...
declare
  ...
  actor_profile_id uuid;
  preview_snapshot jsonb;
  active_final_closing jsonb;
  closing_history jsonb := '[]'::jsonb;
  can_create_interim boolean := false;
  can_close boolean := false;
  can_reopen boolean := false;
  close_blocked_reason text;
begin
  ...
  if active_final_closing is null then
    preview_snapshot := public.build_monthly_settlement_snapshot(
      normalized_period_month
    );
  else
    preview_snapshot := active_final_closing->'snapshot';
  end if;

  can_create_interim := normalized_period_month <= current_period_month
    and active_final_closing is null
    and public.has_permission('settlements.close');

  can_close := normalized_period_month <= current_period_month
    and active_final_closing is null
    and public.has_permission('settlements.close');

  can_reopen := active_final_closing is not null
    and public.has_permission('settlements.reopen')
    and not exists (
      select 1
      from public.monthly_closings as later_closings
  ...
  close_blocked_reason := case
    when active_final_closing is not null then 'already-closed'
    when normalized_period_month > current_period_month then 'future-period'
    when not public.has_permission('settlements.close') then 'permission-required'
    else null
  end;
...
create or replace function public.create_interim_monthly_settlement(
  requested_period_month date
)
returns jsonb
language plpgsql
...
begin
  ...
  if normalized_period_month > current_period_month then
    raise exception 'future month cannot be closed'
      using errcode = '55000';
  end if;
  ...
  if exists (
    ...
    where active_closings.period_month = normalized_period_month
      and active_closings.closing_kind = 'final'
      and active_closings.status = 'closed'
  ) then
    raise exception 'monthly settlement is already finally closed'
      using errcode = '55000';
  end if;
...
declare
  actor_profile_id uuid;
  selected_closing public.monthly_closings%rowtype;
begin
  if requested_closing_id is null then
  ...
  select closings.*
  into selected_closing
  from public.monthly_closings as closings
  where closings.id = requested_closing_id
    and (
      (
        closings.closing_kind = 'interim'
        and closings.status = 'closed'
      )
      or (
        closings.closing_kind = 'final'
        and closings.status in ('closed', 'reopened')
      )
    )
  for update;

  if not found then
    raise exception 'monthly settlement closing not found'
      using errcode = 'P0002';
  end if;
  ...
  values (
    ...
    'monthly_closings',
    selected_closing.id::text,
    pg_catalog.jsonb_build_object(
      'period_month', selected_closing.period_month,
      'closing_kind', selected_closing.closing_kind,
      'version', selected_closing.version,
      'status', selected_closing.status
    ),
    pg_catalog.clock_timestamp()
  );

  return pg_catalog.jsonb_build_object(
    'id', selected_closing.id,
    'period_month', selected_closing.period_month,
    'closing_kind', selected_closing.closing_kind,
    'version', selected_closing.version,
    'status', selected_closing.status,
    'snapshot', selected_closing.snapshot,
    'closed_at', selected_closing.closed_at,
    'closed_by', selected_closing.closed_by_name,
    'reopened_at', selected_closing.reopened_at
  );
end;
$$;
...
from public, anon, authenticated, service_role;
grant execute on function public.create_interim_monthly_settlement(date)
to authenticated;

revoke execute on function public.close_monthly_settlement(date)
from public, anon, authenticated, service_role;
grant execute on function public.close_monthly_settlement(date)
to authenticated;

revoke execute on function public.reopen_monthly_settlement(date)
from public, anon, authenticated, service_role;
grant execute on function public.reopen_monthly_settlement(date)
to authenticated;

revoke execute on function public.record_monthly_report_generation(uuid)
from public, anon, authenticated, service_role;
grant execute on function public.record_monthly_report_generation(uuid)
to authenticated;
