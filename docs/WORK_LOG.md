...
- `/dashboard` 구간에 원문 예외를 노출하지 않는 `대시보드를 불러오지 못했습니다` 오류 경계와 `다시 시도` 복구 작업을 추가했다. 공용 `Button`의 기존 `onClick` 계약과 프로젝트 토큰을 사용한다.
...
- 루트의 무시된 `.env.local`을 값 출력 없이 프로세스에 로드한 최종 `npm run build` — Next.js 16.2.10 Turbopack 컴파일 3.0초, TypeScript 3.2초, 26/26개 정적 페이지 생성, 종료 코드 0.
...
### 브라우저 및 디자인 QA
- 승인 소스 `/Users/seikpark/Desktop/projects/jwtennisclub/.superpowers/brainstorm/32523-1785542447/content/financial-dashboard-layout.html`과 구현을 같은 인앱 브라우저에서 1440×900 및 375×812, DPR 1로 캡처하고 한 비교 보드에 함께 배치해 확인했다. 결과와 반복 이력은 루트 `design-qa.md`에 있으며 `final result: passed`다.
...
- 인증된 실제 로컬 세션으로 `/dashboard`에 접근했을 때 운영 Supabase에 새 RPC가 없어 대시보드 요청이 실패했고, 새 제어 오류 경계가 정상 표시됐다. 해당 화면과 처리된 콘솔 오류를 배포 게이트 증거로 보존했다.
