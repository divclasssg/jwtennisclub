...
### 회원 DB 리뷰 후속 보강
- 운영에서 이미 적용된 `202607270002` 회원번호 교정 마이그레이션을 재실행 안전한 검증 전용으로 변경했다. 기존 `#/25`·`#0024` 1건·`#0020`~`#0023` 공백 조합은 다른 정상 DB에서도 성립할 수 있으므로 더 이상 회원번호, 정모 스냅샷, 할당기를 자동 변경하지 않는다.
- 이미 교정된 `#0020`·할당기 `#/21` 상태는 정모 명단·출석 스냅샷까지 검증하고, 불일치하면 트랜잭션을 실패시킨다. 그 외 상태는 읽기 전용으로 기록하고 그대로 둔다.
- PGlite 기반 인메모리 PostgreSQL 실행 테스트를 추가해 마이그레이션 SQL을 실제로 컴파일·실행한다. 회원번호 교정은 정상 유사 상태 무변경, 적용 완료 재실행, 스냅샷 불일치 롤백을 검증한다.
- 회원 저장 RPC 마이그레이션은 실제 PostgreSQL 역할 ACL, `members.create`, `members.update`, `members.contacts.manage` 권한 조합, 연락처 저장·보존과 한정된 `member_id` 수정 경로를 실행 검증한다.
- 이 변경은 운영 DB에 새 SQL을 적용하거나 운영 데이터를 수정하지 않는다. 운영의 `202607270002` 적용 이력과 이미 확인된 `#0020`·할당기 `#/21` 상태는 그대로 유지한다.
- RED 실행에서 정상 유사 상태의 `#0024` 회원·정모 스냅샷이 실제로 `#0020`으로 바뀌고 할당기가 25에서 21로 되감겨 테스트 1건이 의도한 이유로 실패했다. 검증 전용 변경 뒤 회원번호 실행 테스트 3건과 회원 저장 실행 테스트 5건이 통과했다.
- 최종 전체 Vitest는 111개 파일, 765개 테스트가 통과했다. `npm run lint`, `npx tsc --noEmit`, `git diff --check`도 종료 코드 0으로 통과했다.
- 제한 환경의 첫 프로덕션 빌드는 최적화 단계에서 출력 없이 정체되어 중지했고 통과로 처리하지 않았다. 같은 환경 변수로 네트워크 허용 환경에서 다시 실행한 Next.js 16.2.10 빌드는 컴파일 3.4초, TypeScript 3.0초, 정적 페이지 26/26 생성을 포함해 종료 코드 0으로 통과했다.
