...
- [x] 원격 전용 마이그레이션 drift, 모든 선행 마이그레이션·활동 시작 월 백필, 프로덕션 빌드 게이트가 모두 통과한 뒤 호환성 순서대로 `202607310001_add_interim_monthly_closings.sql`을 먼저 적용·검증하고 `202607310002_lock_finalized_month_sources.sql`을 적용·검증한다
- [x] 검증된 새 결산 앱을 `main`에 배포한다
