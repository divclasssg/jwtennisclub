...
import { formatDate, formatMemberDirectoryKind, formatMemberPosition, formatMemberStatus, formatMemberStatusTab, formatPauseStartMonth } from "@/features/members/member-list";
  ...
  return <ManagementPageTemplate
    ...
    list={<DataPanel aria-label="회원 목록" empty={<EmptyState description="검색어나 필터를 조정하세요." title="표시할 회원이 없습니다" />}
      ...
      {sortedMembers.length ? <><div className={styles["members-table-view"]}><DataTable><thead><tr>
        ...
        <td className={styles["member-code-cell"]}>{member.memberCode}</td><th scope="row">{member.name}</th><td>{member.phoneDisplay}</td><td>{formatMemberDirectoryKind(member)}</td><td>{formatMemberPosition(member)}</td><td>{member.groupCode ?? "없음"}</td><td>{formatMemberStatus(member.status)}</td><td>{formatPauseStartMonth(member.status === "paused" ? member.pauseStartMonth : null)}</td><td>{formatDate(member.joinedDate)}</td><td>{canUpdate ? <Link href={`/members/${member.id}/edit`}>수정</Link> : null}</td>
